            ...
            <div class="card">
                    ...
                    <div class="row justify-content-center pt-5">
                      <div class="col-md-4 order-md-2">
                        <strong  class="total float-md-end">
                          <span id="valeur_total_operation">{{ $operation->total_operation }}</span>&nbsp;€
                        </strong>
                      </div>
                      <div class="col-md-6">
                        <div class="mb-3">
                          ...
                          <select id="disabledSelect" name="type_operation" class="form-select">
                            @foreach($type_operations as $type_operation)
                              <option value="{{ $type_operation['id'] }}" @if ($type_operation['id'] == $operation->type_operation) selected @endif>{{ $type_operation['title'] }}</option>
                            @endforeach
                          </select>
                        </div>
                        <div class="mb-3">
                          <label for="date" class="form-label">Date</label>
                          <input type="date" name="date_operation" value="{{ $operation->date_operation }}" class="form-control" id="date">
                        </div>
                      </div>
                    </div>
                    <div class="row justify-content-center">
                      <div class="col-md-10">
                        <div class="mb-3">
                          ...
                          <textarea id="note" name="note_operation" class="form-control">{{ $operation->note_operation }}</textarea>
                        </div>
                      </div>
                    </div>
                </div>
                ...
                <div class="card-body">
                  <div class="row justify-content-center pt-5 pb-5">
                    <div class="col-auto">
                      <input type="hidden" name="total_operation" id="total_operation" value="{{$operation->total_operation}}"/>
